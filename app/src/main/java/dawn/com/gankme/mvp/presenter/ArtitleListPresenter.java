...
import dawn.com.gankme.mvp.ui.fragment.WebFragment;
 ...
 */
@FragmentScope
public class ArtitleListPresenter extends BasePresenter<ArticleListContract.Model, ArticleListContract.View> {
    ...
    @Inject
    public ArtitleListPresenter(ArticleListContract.Model model, ArticleListContract.View rootView, RxErrorHandler handler
            , AppManager appManager, Application application, List<Article> list, RecyclerView.Adapter adapter) {
    ...
    public void requestgetRecently(final boolean pullToRefresh) {
        ...
        if (pullToRefresh) pageIndex = 0;//下拉刷新默认只请求第一页
        ...
        mModel.getRecently(pageIndex, isEvictCache)
                ...
                .subscribe(new ErrorHandleSubscriber<List<Article>>(mErrorHandler) {
                    ...
                    @Override
                    public void onNext(List<Article> datas) {
                        if(datas.size()<=0){
                        ...
                        ++pageIndex;
                        preEndIndex = articles.size();//更新之前列表总长度,用于确定加载更多的起始位置
    ...
    public void onItemClick(int position) {
        ...
        ((ArticleListFragment)mRootView).start(WebFragment.newInstance(url, title,id));
